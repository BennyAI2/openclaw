...
async function restoreGatewayService(reason) {
  // Once triage owns cancellation, an updater failure must never resurrect a stopped Gateway.
  ...
  if (recovery?.kind === "systemd") {
    ...
    for (let inspection = enabled; enabled.code === 0 && Date.now() < deadline;) {
      ...
      if (
        inspection.code === 130 ||
        /already exists in domain|operation already in progress|bootstrap failed: 37/i.test(detail)
      )
        continue;
    ...
    restored =
      (await runServiceCommand("schtasks.exe", ["/Run", "/TN", recovery.taskName])).code === 0;
  }
