...
export function createAgentEventHandler({
  ...
  const handleEvent = (event: AgentEventPayload) => {
    ...
    if (lifecyclePhase === "error") {
      ...
      const isDefinitive = isDefinitiveRunLifecycle({ phase: lifecyclePhase, data: evt.data });
      // Only retryable failures retain grace. Native cancellation and timeout
      // are definitive even when no chat-abort marker was created.
      if (isAborted || isDefinitive || lifecycleErrorRetryGraceMs <= 0) {
