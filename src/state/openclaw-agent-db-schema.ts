...
function ensureAgentSchema(
  ...
  if (preMigrationBackup.status === "created") {
    agentDbLog.info(describePreMigrationSnapshot(preMigrationBackup));
  }
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (
        ...
        previousVersion !== preMigrationBackup.fromVersion
      ) {
        throw new Error(`Agent database schema changed after its backup was created: ${pathname}`);
      }
