...
export async function handleCompactionStart(turn: Turn): Promise<boolean> {
  const progress = turn.progressCompositor.pushToolProgress(
    buildTelegramCompactionProgressLine("start"),
    { startImmediately: true },
  );
...
export async function handleCompactionEnd(
  ...
  const progress = turn.progressCompositor.pushToolProgress(
    buildTelegramCompactionProgressLine(payload?.completed === false ? "incomplete" : "complete"),
    { startImmediately: true },
  );
