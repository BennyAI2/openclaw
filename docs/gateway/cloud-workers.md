...
Ready workers incur running-machine charges. Their fixed expiry starts at the actual project demand that triggered preparation and uses the provider's existing idle policy: `settings.idleTimeout` for Crabbox. This is separate from `suspendAfter`, which applies to an assigned idle session. Refills, retries, inspections, and heartbeats do not extend reserve expiry. Periodic maintenance requests teardown even without another dispatch; uncertain deletion remains recorded and consumes capacity. A Gateway or cleanup-owner outage can delay physical deletion and continue charges, so provider TTL settings are not a uniform guarantee of autonomous cleanup across backends.
