...
Provider cards expose every manifest-declared provider access method. **Sign in** runs credential-only browser, device-code, API-key, and token flows without changing or testing the default model. **Set up** runs endpoint, local-runtime, and complex cloud configuration through the same guided dialog while preserving the current default model.
