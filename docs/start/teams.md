...
Teammates with their own model subscriptions can link them under **Settings → Profile → Model accounts** so sessions they start bill their own ChatGPT or Claude account instead of one shared credential - see [Per-person model accounts](/concepts/multi-user#per-person-model-accounts).
